...
int init_module(void)
{
    ...
    if (!dir) {
        // Abort module load.
        printk(KERN_ALERT "debugfs_example1: failed to create /sys/kernel/debug/example\n");
        return -1;
    }

    // Create file `/sys/kernel/debug/example/hello`.
    ...
    if (!junk) {
        // Abort module load.
        printk(KERN_ALERT "debugfs_example1: failed to create /sys/kernel/debug/example/hello\n");
        return -1;
    }
